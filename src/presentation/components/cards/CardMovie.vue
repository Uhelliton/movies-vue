...
<template>
  <div class='flex max-w-xs w-full bg-white shadow-md rounded-lg overflow-hidden mx-auto'>
    <div class="overflow-hidden rounded-xl relative transform hover:-translate-y-2 transition ease-in-out duration-500 shadow-lg hover:shadow-2xl" data-movie-id="438631">
        ...
        <div class="align-self-end w-full">
          ...
          <div class="space-y-6">
            <div class="flex flex-col space-y-2 inner">
              <h3 class="text-2xl font-bold text-white" data-unsp-sanitized="clean">Duna {{ title }}</h3>
              <div class="mb-0 text-xs text-gray-400">uma simples descrição aqui. {{ description }}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="flex flex-row relative pb-4 space-x-4 z-10">
        <a
          class="flex items-center py-1 px-2 mt-2 rounded-full mx-auto text-white bg-red-500 hover:bg-red-700"
          ...
          <div class="text-xs text-white ml-2">11 de mai de 2023</div>
        </a>
      </div>
    </div>
  </div>
